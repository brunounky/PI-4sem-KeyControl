...
function buildQuery($contrato_id, $locatario_nome, $contrato_vigencia, $contrato_dia_vencimento, $tipo_imovel, $contrato_forma_pagamento) {
    $sql = "
        SELECT DISTINCT 
            ...
            cf.contrato_vigencia, 
            cf.contrato_dia_vencimento,
            cf.contrato_forma_pagamento
        FROM 
            contrato_fiador cf
        JOIN 
            cadastro_cliente cc ON cf.comprador_cpf_cnpj = cc.cpf_cnpj 
        JOIN 
            cadastro_imovel ci ON cf.imovel_proprietario_cpf_cnpj = ci.cpf_cnpj_proprietario 
        JOIN 
    ...
    if (!empty($contrato_id)) {
        $sql .= " AND cf.id = :contrato_id"; 
    }
    if (!empty($locatario_nome)) {
        $sql .= " AND cf.locatario_nome = :locatario_nome"; 
    }
    if (!empty($contrato_vigencia)) {
        $sql .= " AND cf.contrato_vigencia = :contrato_vigencia"; 
    }
    if (!empty($contrato_dia_vencimento)) {
        $sql .= " AND cf.contrato_dia_vencimento = :contrato_dia_vencimento"; 
    }
    if (!empty($tipo_imovel)) {
        $sql .= " AND ci.tipo_imovel = :tipo_imovel"; 
    }
    if (!empty($contrato_forma_pagamento)) {
        $sql .= " AND cf.contrato_forma_pagamento = :contrato_forma_pagamento"; 
    }
